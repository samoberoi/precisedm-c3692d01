...
const SubscriptionPage = () => {
  ...
  const handleStartTrial = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("subscriptions").insert({
        user_id: user.id, plan_type: "trial", status: "active",
        start_date: new Date().toISOString(), next_billing_date: trialEnd.toISOString(),
      });
    ...
    } finally { setTrialProcessing(false); }
  };
  ...
  const handleSubscribe = async (planType: string) => {
    ...
    try {
      ...
      if (!accessToken) { const { data } = await supabase.auth.getSession(); accessToken = data.session?.access_token; }
      ...
      toast({ title: "Error", description: err instanceof Error ? err.message : "Failed to create subscription.", variant: "destructive" });
    } finally { setProcessing(false); setSelectedPlan(null); }
  };

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-background pb-36 md:pb-16">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between px-5 pt-12 pb-3 md:pt-8">
          ...
          <h1 className="text-lg md:text-2xl font-bold text-foreground md:w-full">Subscription</h1>
        ...
        <div className="px-5 pt-3">
          ...
          {!isActive && !hasUsedTrial && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
              ...
              <Button className="w-full h-12 rounded-2xl font-bold text-sm bg-white text-purple-700 hover:bg-white/90" disabled={trialProcessing} onClick={handleStartTrial}>
